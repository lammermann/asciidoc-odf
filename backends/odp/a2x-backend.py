...
# a2x extension for odp backend
#
# backend options
#
# --base_doc=path  The odp document to use for styles etc.
...
class odp_archive:
    ...
    def make_archive(self, base_doc, doc, out_file, schema):
        with tempdir(self.temp, self.keep) as td:
            ...
            shell( '%s --backend=odp -a "a2x-format=%s" -a not_flat_odf %s --out-file "%s" "%s"' %
...
def to_odp(self):
    ...
    opts = AttrDict(base_doc=os.path.join(getpardir(), 'asciidoc.otp'),
    ...
    odf_file = self.dst_path('.odp')
    a = odp_archive(self, opts.temp_dir, self.keep_artifacts)
